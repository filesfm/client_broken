...
private slots:
    ...
    void testSeparateUpload()
    {
        ...
        // as usual.
        fakeFolder.localModifier().setContents(conflictName, FileModifier::DefaultFileSize, 'P');
        QVERIFY(fakeFolder.applyLocalModificationsAndSync());
        QCOMPARE(conflictMap.size(), 1);
        QCOMPARE(conflictMap[a1FileId], conflictName);
        QCOMPARE(fakeFolder.currentLocalState(), fakeFolder.currentRemoteState());
        ...
        fakeFolder.remoteModifier().setContents(conflictName, FileModifier::DefaultFileSize, 'Q');
        QVERIFY(fakeFolder.applyLocalModificationsAndSync());
        QCOMPARE(fakeFolder.currentLocalState(), fakeFolder.currentRemoteState());
