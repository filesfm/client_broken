...
private slots:
    ...
    void testFakeConflict()
    {
        ...
        fakeFolder.localModifier().setContents("A/a1", FileModifier::DefaultFileSize, 'C');
        fakeFolder.localModifier().setModTime("A/a1", mtime);
        fakeFolder.remoteModifier().setContents("A/a1", FileModifier::DefaultFileSize, 'C');
        if (!sameMtime) {
    ...
    void testUploadV1Multiabort()
    {
        ...
        QTimer::singleShot(200, &fakeFolder.syncEngine(), [&]() { fakeFolder.syncEngine().abort(); });
        QVERIFY(!fakeFolder.applyLocalModificationsAndSync());
