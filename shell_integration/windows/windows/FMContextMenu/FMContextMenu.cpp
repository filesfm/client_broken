...
FMContextMenu::FMContextMenu(void) 
    : m_cRef(1)
{
...
IFACEMETHODIMP FMContextMenu::QueryInterface(REFIID riid, void **ppv)
{
    static const QITAB qit[] =
    {
        QITABENT(FMContextMenu, IContextMenu),
        QITABENT(FMContextMenu, IShellExtInit),
        { 0 },
    };
...
IFACEMETHODIMP_(ULONG) FMContextMenu::AddRef()
{
    return InterlockedIncrement(&m_cRef);
}

// Decrease the reference count for an interface on an object.
IFACEMETHODIMP_(ULONG) FMContextMenu::Release()
{
...
IFACEMETHODIMP FMContextMenu::Initialize(
...
IFACEMETHODIMP FMContextMenu::QueryContextMenu(HMENU hMenu, UINT indexMenu, UINT idCmdFirst, UINT idCmdLast, UINT uFlags)
{
    ...
    m_info = FMClientInterface::FetchInfo(m_selectedFiles);
...
IFACEMETHODIMP FMContextMenu::InvokeCommand(LPCMINVOKECOMMANDINFO pici)
{
    std::wstring command;
    ...
    FMClientInterface::SendRequest(command, m_selectedFiles);
    return S_OK;
}

IFACEMETHODIMP FMContextMenu::GetCommandString(UINT_PTR idCommand,
